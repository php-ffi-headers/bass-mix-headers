...
final class HeadersDownloader
{
    ...
    {
        return \vsprintf('https://www.un4seen.com/files/bassmix%s.zip', [
    ...
    {
        return \sys_get_temp_dir() . '/bass-mix-headers-' . $version->toString() . '.zip';
    }
    ...
    public static function download(VersionInterface $version, string $directory): void
    {
        ...
        foreach (self::readArchive($version) as $file) {
            if ($file->isFile() && \str_ends_with($file->getPathname(), 'c/bassmix.h')) {
